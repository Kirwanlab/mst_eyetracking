...
###??? update these
workDir=~/fsl_groups/fslg_KirwanLab/compute/encoding_fmri_new/derivatives/$subj
dataDir=~/fsl_groups/fslg_KirwanLab/compute/encoding_fmri_new/rawdata/$subj
tempDir=~/fsl_groups/fslg_KirwanLab/compute/encoding_fmri_new/code/templates
template=${tempDir}/vold2_mni_brain+tlrc
priorDir=${tempDir}

blip=0                                  # blip toggle (1=on)
phaseArr=(contrecog)                          # Each PHASE of experiment, in order (TEST1 precedes TEST2 but followed STUDY). This is absolutely necessary - so put something here
blockArr=(5)                            # number of blocks (runs) in each phase. E.g. STUDY had 1 block, TEST1 had 2 blocks. INTEGER!
phaseLen=${#phaseArr[@]}
